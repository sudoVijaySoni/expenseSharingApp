...
const displayExpense = async(req, res) => {
    ...
    //fetching list of members in current group 
    ...
    for(var i=0; i<usersFname.length; i++){
        for(var j=0; j<usersFname.length; j++){
            if(i==j)
                ...
                //calculating sum of all amounts
                var totalAmount = 0;
                for(var k=0;k<currentExpense.length;k++){
                    if(currentExpense[k].from.fname==usersFname[i] && currentExpense[k].to.fname==usersFname[j]){
